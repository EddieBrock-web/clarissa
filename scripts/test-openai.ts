 ...
 * Run with: bun scripts/test-openai.ts
...
if (!apiKey) {
  ...
  console.log(dim("Add OPENAI_API_KEY=your-key to .env file"));
  process.exit(1);
}
...
async function testAvailability(): Promise<boolean> {
  ...
  if (status.available) {
    pass(`OpenAI is available`);
    if (status.model) info(`Model: ${status.model}`);
    return true;
  } else {
    fail(`OpenAI is not available: ${status.reason}`);
    return false;
  }
}
...
async function main() {
  console.log("\n" + cyan("OpenAI Provider - Manual Test Suite"));
